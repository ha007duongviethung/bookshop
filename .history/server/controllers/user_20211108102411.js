...
module.exports = class UserAPI {
    ...
    static async fetchUser(req, res) {
        ...
        try {
            ...
            const full_name = awai.full_name
            const type = await user.type
            if(type)
                res.json({success: true, message: "Admin logged in successfully", accessToken: authorization, full_name})
            else
                res.json({success: true, message: "User logged in successfully", accessToken: authorization, full_name})
        } catch (error) {
            console.error(error)
            res.status(500).json({success: false, message: "Internal Server Error"})
        }
