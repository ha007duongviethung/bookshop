            ...
            <li class="nav-item dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                <li v-for="authorHeader in authorShow" :key="authorHeader">
                  <a class="dropdown-item" :href="`/author-${removeVietnameseTones(authorHeader)}`">{{authorHeader}}</a>
                </li>
                <!-- <li><hr class="dropdown-divider" /></li>
                <li>
            ...
            <li class="nav-item dropdown">
              ...
              >
                ...
                <div style="width: 40px; height: 40px">
                  <img :src="avatar = (avatar) ? avatar : 'https://w7.pngwing.com/pngs/627/693/png-transparent-computer-icons-user-user-icon-thumbnail.png'" style="width: 100%; height: 100%; object-fit:cover" alt="">
                </div>
              </a>
              <ul
                class="dropdown-menu"
...
export default {
  ...
  methods: {
    ...
    async handleSearch() {
      const data = (await BookAPI.getAllBooks()).filter(item => item.book_name.toLowerCase().search(this.search.toLowerCase()) != -1)
      this.$router.push({ path: '/result-search', params: data })
    }
  },
  async created() {
    this.token()
