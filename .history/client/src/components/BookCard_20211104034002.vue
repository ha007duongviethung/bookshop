<template>
  <div class="col-12 col-sm-6 col-md-4 col-lg-3 col-xl-2">
    <div class="card mb-3">
      <router-link :to="`/book/detail/${book._id}`" class="card-img">
        <img :src="`http://localhost:305/${book.img}`" class="card-img-top" alt="book" />
        <div class="quick-add">
          <a
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="quick-add_box">
                    ...
                    <div class="quick-add_des">
                      ...
                      <div class="mt-3">
                        {{ book.brief_des.substring(0,150) + "..." }}
                      </div>
                      <div class="mt-3">
                        <button class="btn btn-primary">Thêm vào giỏ hàng</button>
                        <button class="btn btn-outline-dark mt-2">Đăng nhập để mua ngay</button>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="modal-footer pt-4">
                  <button class="btn btn-success mx-auto w-100 view-detail" 
                    data-bs-dismiss="modal"
                    aria-label="Close">
                    <router-link :to="`/book/detail/${book._id}`">Xem chi tiết sách >></router-link>
                  </button>
                </div>
            </div>
          </div>
      ...
      <div class="card-body">
        <router-link :to="`/book/detail/${book._id}`" class="card-title">{{ book.book_name }}</router-link>
        <p class="card-text">{{ book.type.substring(0, 15) + "..." }}</p>
        <p class="card-text">
          <small class="text-muted">by: {{ book.author.substring(0, 12) + "..." }}</small>
        </p>
      </div>
    </div>
  </div>
