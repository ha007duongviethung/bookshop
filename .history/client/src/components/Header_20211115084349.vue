            ...
            <li class="nav-item dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                <li v-for="authorHeader in authorShow" :key="authorHeader">
                  <span class="dropdown-item" @click="author">{{authorHeader}}</span>
                </li>
                <!-- <li><hr class="dropdown-divider" /></li>
                <li>
...
export default {
  ...
  methods: {
    ...
    async author(e) {
      const at = e.target.innerText
      // const data = []
      // this.bookapi.forEach(item => {
      //   if(item.author == at)
      //     data.push(item)
      // })
      this.$router.push({name: 'AuthorDescription', params: {name: at}})
    }
  },
  async created() {
    this.token()
    ...
    this.bookapi.forEach(book => {
      ...
      const temp = slug + "/" + type
      allType[temp] = allType[temp] ? allType[temp] + 1 : 1

      const tmp = book.author
      allAuthor[tmp] = allAuthor[tmp] ? allAuthor[tmp] + 1 : 1
    })
