<template>
    <tr>
        <td>
            <span class="custom-checkbox">
                <input type="checkbox" id="checkbox1" value="options[]">
                <label for="checkbox1"></label>
            </span>
        </td>
        <td class="book-item">
            <img class="book-item-img" :src="`http://localhost:305/${book.img}`" onerror="this.src='data:image/png;base64,'" alt="">
            {{book.book_name}}
        </td>
        <td>{{book.author}}</td>
        <td>{{book.pb_company}}</td>
        <td>{{book.type}}</td>
        <td>{{book.create_at}}</td>
        <td>
            <a href="" class="edit" data-bs-toggle="modal" :data-bs-target="`#ed`+book._id"><i class="material-icons" data-bs-toggle="tooltip" title="Edit">&#xE254;</i></a>
            <a href="" class="delete" data-bs-toggle="modal" data-bs-target="#deleteEmployeeModal"><i class="material-icons" data-bs-toggle="tooltip" title="Delete">&#xE872;</i></a>
        </td>
        <div :id="`ed`+book._id" class="modal fade" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog large">
                <div class="modal-content">
                    <form @submit.prevent="handleUpdateBook">
                        <div class="modal-header">						
                            <h4 class="modal-title">Sửa sách</h4>
                            <button
                            type="button"
                            class="btn-close"
                            data-bs-dismiss="modal"
                            aria-label="Close"
                            @click="reset"
                            ></button>
                        </div>
                        <div class="modal-body">					
                            <div class="row">
                                <div class="col">
                                    <div class="form-group">
                                        <label for="book_name">Tên sách</label>
                                        <input type="text" class="form-control" id="book_name" v-model="book.book_name" required>
                                    </div>
                                </div>
                                <div class="col">
                                    <div class="form-group">
                                        <label for="author">Tên tác giả</label>
                                        <input type="text" class="form-control" id="author" v-model="book.author" required>
                                    </div>
                                </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col">
                                    <div class="form-group">
                                        <label for="pb_company">Nhà sản xuất</label>
                                        <input type="text" class="form-control" id="pb_company" v-model="book.pb_company" required>
                                    </div>
                                </div>
                                <div class="col">
                                    <div class="form-group">
                                        <label for="tp">Thể loại</label>
                                        <input type="text" class="form-control" id="tp" v-model="book.type" required>
                                    </div>
                                </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col">
                                    <div class="form-group">
                                        <label for="price">Giá sách (VNĐ)</label>
                                        <input type="number" class="form-control" id="price" v-model="book.price" required>
                                    </div>
                                </div>
                                <div class="col">
                                    <div class="form-group">
                                        <label for="amount">Số lượng</label>
                                        <input type="number" class="form-control" id="amount" v-model="book.amount" required>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group mt-3">
                                <label for="brief_des">Mô tả ngắn gọn</label>
                                <textarea class="form-control" id="brief_des" v-model="book.brief_des"></textarea>
                            </div>
                            <div class="form-group mt-3">
                                <label for="detailed_des">Mô tả chi tiết</label>
                                <textarea class="form-control" id="detailed_des" v-model="book.detailed_des"></textarea>
                            </div>
                            <div class="row mt-3">
                                <div class="col">
                                    <div class="form-group">
                                        <label for="height">Chiều dài (cm)</label>
                                        <input type="number" class="form-control" id="height" v-model="book.height">
                                    </div>
                                </div>
                                <div class="col">
                                    <div class="form-group">
                                        <label for="width">Chiều rộng (cm)</label>
                                        <input type="number" class="form-control" id="width" v-model="book.width">
                                    </div>
                                </div>
                                <div class="col">
                                    <div class="form-group">
                                        <label for="weight">Cân nặng (g)</label>
                                        <input type="number" class="form-control" id="weight" v-model="book.weight">
                                    </div>
                                </div>
                            </div>
                            <div class="input-group mt-3">
                                <input type="file" v-on:change="uploadImage" class="form-control" id="inputGroupFile02">
                                <label class="input-group-text" for="inputGroupFile02">Upload</label>
                            </div>				
                        </div>
                        <div class="modal-footer">
                            <input type="button" class="btn btn-default" data-bs-dismiss="modal" value="Cancel">
                            <input type="submit" class="btn btn-success" value="Sửa sách">
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </tr>
</template>

<script>
import BookAPI from "../api/book"

export default {
    name: "BookItem",
    props: ["book"],
    data: () => {
        return {

        }
    },
    methods: {
        async handleUpdateBook() {
            console.log(this.book)
            await BookAPI.updateBook(this.book._id, this.book)
        }
    }
}
</script>