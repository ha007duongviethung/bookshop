<template>
  <div class="col-12 col-sm-6 col-md-4 col-lg-3 col-xl-2">
    <div class="card mb-3">
      <a :href="`/book/detail/${book._id}`" class="card-img">
        <img :src="`http://localhost:305/${book.img}`" class="card-img-top" alt="book" />
        <div class="quick-add">
          <a
            data-bs-toggle="modal"
            :data-bs-target="`#qa`+book._id"
            >Quick add</a
          >
        </div>        
        <div
            class="modal fade"
            :id="`qa`+book._id"
            tabindex="-1"
            aria-hidden="true"
            @click.stop.prevent
          >
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                  <button
                    type="button"
                    class="btn-close"
                    data-bs-dismiss="modal"
                    aria-label="Close"
                  ></button>
                </div>
                <div class="modal-body">
                  <div class="quick-add_box">
                    <img :src="`http://localhost:305/${book.img}`" onerror="this.src='data:image/png;base64,'" class="card-img-top" alt="book" />
                    <div class="quick-add_des">
                      <h3>{{ book.book_name }}</h3>
                      <p>{{ book.author }}</p>
                      <p>{{ book.type }}</p>
                      <div>
                        <span class="new-price">${{ book.price }}</span>
                        <span class="old-price">${{ book.price * 1.5}}</span>
                      </div>
                      <div class="mt-3">
                        {{ book.brief_des.substring(0,150) + "..." }}
                      </div>
                      <div class="mt-3">
                        <button class="btn btn-primary">Thêm vào giỏ hàng</button>
                        <button class="btn btn-outline-dark mt-2">Đăng nhập để mua ngay</button>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="modal-footer pt-4">
                  <button class="btn btn-success mx-auto w-100 view-detail" 
                    data-bs-dismiss="modal"
                    aria-label="Close">
                    <a :href="`/book/detail/${book._id}`">Xem chi tiết sách >></a>
                  </button>
                </div>
            </div>
          </div>
        </div>
      </a>
      <div class="card-body">
        <a :href="`/book/detail/${book._id}`" class="card-title">{{ book.book_name }}</a>
        <p class="card-text">{{ book.type.substring(0, 15) + "..." }}</p>
        <p class="card-text">
          <small class="text-muted">by: {{ book.author.substring(0, 12) + "..." }}</small>
        </p>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "BookCard",
  props: ["book"]
};
</script>
